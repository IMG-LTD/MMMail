...
- GitHub CLI checks must see a successful `push` event run for the `MMMail Images` workflow, GitHub Release notes for the same tag containing both image names and immutable digests, GHCR backend/frontend-admin package versions, and an accessible `IMG-LTD/mmmail-billing-gateway` repository. GHCR package version checks require a `gh` token with `read:packages`; a GitHub API 403 is a permissions evidence gap, not proof that the package does not exist.
...
- `v2.2 image digest release notes are not visible`
...
If the GHCR rows fail with HTTP 403 or GitHub's `read:packages` message, the missing item remains visible until package metadata can be read with an appropriately scoped token. The verifier still retains publication precondition, release-note, and workflow visibility checks for regressions. The visible rc10 through rc13 `MMMail Images` successes are historical baseline evidence only; final image digest acceptance must come from a tag, GitHub Release notes, and workflow tied to the same Public MMMail commit as the other completed evidence packages.
