# MMMail v2.0.3 数据迁移策略

**版本**: `v2.0.3`
**日期**: `2026-04-23`

## 目标
- 使用 Flyway 版本化迁移统一新部署与存量库升级入口。
- 保持当前 `main` / `v2.0.3` 公开基线的 schema 可验证、可升级、可恢复。
...
- 对已存在业务表、但没有 `flyway_schema_history` 的数据库：
  ...
  - 然后继续执行 `V2+` 的幂等迁移
- 这样可以同时兼容已执行过旧初始化脚本但未纳入 Flyway 管理的历史库。
...
- `backend/mmmail-server/src/test/java/com/mmmail/server/MigrationTestVersions.java`
