...
| Frontend Lighthouse toolchain dependency | `frontend-admin/package.json`, `frontend-admin/pnpm-lock.yaml`, `frontend-admin/scripts/run-lighthouse.mjs`, `frontend-admin/tests/v212-browser-gates-contract.test.mjs`, `tests/v22-ci-toolchain-contract.test.mjs` | `scripts/run-lighthouse.mjs` directly imports `chrome-launcher`, so the package declares `chrome-launcher@1.2.1` as a direct devDependency instead of relying on Lighthouse transitive dependencies; the script now explicitly uses Lighthouse `preset: 'desktop'` while keeping the `> 80` performance threshold | Done |
| Backend dependency security baseline | `backend/pom.xml`, `DependencyVersionGuardTest`, `tests/v22-ci-toolchain-contract.test.mjs` | Remote rc5 validate exposed OWASP dependency-check failures for high-risk runtime dependencies; the backend now pins or resolves Spring Boot 3.5.14, Tomcat 10.1.55, Spring Security 6.5.10, Log4j 2.26.0, Netty 4.1.133.Final, OpenTelemetry semconv 1.41.1, and Kotlin stdlib 2.3.21, with runtime guards so future scan drift is visible before release | Done |
...
| `timeout 900s mvn -f backend/pom.xml org.owasp:dependency-check-maven:aggregate -Dformats=HTML,JSON -Dodc.outputDirectory=/tmp/mmmail-security-local/dependency-check -DdataDirectory=.tools/dependency-check-data -DfailBuildOnCVSS=7 -DautoUpdate=false` | Build success using the cached dependency-check database; HTML and JSON reports were written to `/tmp/mmmail-security-local/dependency-check/`, and no CVSS >= 7 backend dependency finding failed the build |
...
| `pnpm --dir frontend-admin test:v212` | 124 passed, including v2.2 commercial license, billing, OIDC entry, entitlement localization, commercial a11y gate contracts, and the explicit Lighthouse desktop preset contract |
...
| `pnpm --dir frontend-admin test:lighthouse` | Passed after a fresh build, direct `chrome-launcher` dependency declaration, and explicit Lighthouse desktop preset; `/login` Lighthouse performance=84 |
