...
import type { SuiteCollaborationSync } from '~/types/api'
...
type CollaborationSyncStatus = 'IDLE' | 'CONNECTING' | 'CONNECTED' | 'RECONNECTING' | 'ERROR'

interface CollaborationSyncStreamOptions {
  onPayload?: (payload: SuiteCollaborationSync) => void | Promise<void>
}

export function useCollaborationSyncStream(options: CollaborationSyncStreamOptions = {}) {
  const authStore = useAuthStore()
  const orgAccessStore = useOrgAccessStore()
  const runtimeConfig = useRuntimeConfig()
  const status = ref<CollaborationSyncStatus>('IDLE')
  ...
  async function connect(afterEventId?: number): Promise<void> {
    if (!authStore.accessToken) {
      status.value = 'ERROR'
      errorMessage.value = 'Missing access token for collaboration sync'
      return
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Collaboration sync stream failed: HTTP ${response.status}`)
      }
      if (!response.body) {
        throw new Error('Collaboration sync stream returned an empty body')
      }
      status.value = 'CONNECTED'
      await consumeStream(response.body)
      if (!manuallyClosed) {
        scheduleReconnect('Collaboration sync stream closed')
      }
    } catch (error) {
      if (abortController?.signal.aborted && manuallyClosed) {
        return
      }
      const message = error instanceof Error ? error.message : 'Collaboration sync stream failed'
...
function buildStreamUrl(apiBase: string, afterEventId?: number): string {
  ...
  const streamUrl = new URL('/api/v1/suite/collaboration-center/stream', baseUrl)
...
function parseSsePayload(chunk: string): SuiteCollaborationSync | null {
