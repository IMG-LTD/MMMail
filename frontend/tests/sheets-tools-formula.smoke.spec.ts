...
function createWorkbenchState() {
  ...
  return {
    ...
    localGrid: ref([['Roadmap', '=SUM(A1:A1)']]),
    ...
    dirtyCount: ref(2),
    workbookCount: ref(1),
    savedGrid: ref([['Roadmap', '=SUM(A1:A1)']]),
    computedGrid: ref([['Roadmap', '10']]),
    ...
    formulaPreviewHint: ref('Unsaved formula'),
...
describe('sheets tools/formula smoke', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('wires search, sort and freeze actions through the page', async () => {
    const wrapper = await mountPage()
    await flushPromises()

    await wrapper.get('[data-testid="sheets-search-input"]').setValue('budget')
    expect(workbenchState.updateSearchQuery).toHaveBeenCalledWith('budget')

    await wrapper.get('[data-testid="sheets-sort-include-header"]').setValue(false)
    await wrapper.get('[data-testid="sheets-sort-asc"]').trigger('click')
    await wrapper.get('[data-testid="sheets-sort-desc"]').trigger('click')
    expect(workbenchState.onSortSheet).toHaveBeenNthCalledWith(1, {
      direction: 'ASC',
      includeHeader: false,
    })
    expect(workbenchState.onSortSheet).toHaveBeenNthCalledWith(2, {
      direction: 'DESC',
      includeHeader: false,
    })

    await wrapper.get('[data-testid="sheets-freeze-rows"]').trigger('click')
    await wrapper.get('[data-testid="sheets-freeze-cols"]').trigger('click')
    await wrapper.get('[data-testid="sheets-clear-freeze"]').trigger('click')
    expect(workbenchState.onFreezeRowsToActiveCell).toHaveBeenCalledTimes(1)
    expect(workbenchState.onFreezeColsToActiveCell).toHaveBeenCalledTimes(1)
    expect(workbenchState.onClearFreeze).toHaveBeenCalledTimes(1)
  })

  it('wires formula updates through the page', async () => {
    const wrapper = await mountPage()
    await flushPromises()

    await wrapper.get('[data-testid="sheets-formula-input"]').setValue('=A1+B1')
    expect(workbenchState.onFormulaChange).toHaveBeenCalledWith('=A1+B1')
  })
})
