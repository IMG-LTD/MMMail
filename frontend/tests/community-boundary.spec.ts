...
describe('community boundary helpers', () => {
  ...
  it('keeps hosted-only and self-hosted boundary registries non-empty', () => {
    ...
    expect(countCommunityCapabilitiesByStatus('LIMITED')).toBe(1)
    expect(countCommunityCapabilitiesByStatus('DISCOVERY')).toBe(3)
    expect(countCommunityCapabilitiesByStatus('NOT_SHIPPED')).toBe(2)
    expect(countCommunityCapabilitiesByStatus('HOSTED_ONLY')).toBe(1)
    expect(COMMUNITY_BOUNDARY_DOC_PATHS).toContain('README.md')
    expect(COMMUNITY_BOUNDARY_DOC_PATHS).toContain('docs/release/community-v1-v1.2-mainline-roadmap.md')
...
describe('suite release boundary panel', () => {
  it('renders boundary sections and canonical docs', () => {
    ...
    expect(wrapper.text()).toContain('docs/release/community-v1-v1.2-capability-boundaries.md')
  })
})
