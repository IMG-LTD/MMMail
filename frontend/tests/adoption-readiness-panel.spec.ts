...
describe('SettingsAdoptionReadinessPanel', () => {
  ...
  it('renders start-here checklist and concrete resource links', async () => {
    ...
    expect(panel.get('[data-testid="settings-adoption-release-boundary-card"]').text()).toContain('settings.adoption.cards.boundary.title')
    expect(panel.get('[data-testid="settings-adoption-boundary-link"]').attributes('href')).toBe('/suite?section=boundary')
    expect(panel.get('[data-testid="settings-adoption-release-boundary-meta"]').text()).toContain('settings.adoption.cards.boundary.meta')
