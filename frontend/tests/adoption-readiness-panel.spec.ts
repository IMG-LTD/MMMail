...
describe('SettingsAdoptionReadinessPanel', () => {
  ...
  it('renders start-here checklist and concrete resource links', async () => {
    ...
    expect(panel.get('[data-testid="settings-adoption-track-secondary-team_handoff"]').attributes('href')).toBe('/pass')
    expect(panel.get('[data-testid="settings-adoption-track-primary-developer_handoff"]').attributes('href')).toBe('/self-hosted/adoption.html')
