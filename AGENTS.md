...
- `frontend-admin test:e2e` 的 Docker setup 会编译 Java 21 后端，CI frontend job 必须在 e2e 前显式安装 Java 21。
...
- GitHub Dependabot 告警是安全输入，不是提醒性噪音；涉及当前支持范围的 open alert 必须修到告警给出的 fixed version 或更高版本，并用 lockfile、POM 或显式版本 guard 固定。
- 复查公开告警时使用 `gh api repos/IMG-LTD/MMMail/dependabot/alerts?state=open&per_page=100`；如果 GitHub Package / GHCR 证据需要 package metadata，当前 `gh` token 必须具备 `read:packages` scope，否则只能记录权限缺口，不能把不可见当作不存在。
- 前端供应链修复必须更新 `frontend-admin/package.json` 与 `frontend-admin/pnpm-lock.yaml`，并运行 `pnpm --dir frontend-admin install` 后验证 `pnpm --dir frontend-admin why <packages>`，避免只改 lockfile 不更新本地依赖图。
- 后端供应链修复必须同步 `DependencyVersionGuardTest` 或等价 contract；安全依赖下限不能只存在于 completion audit 或一次性 dependency-check 输出。
...
| 供应链安全 / Dependabot 告警 | `node --test tests/v22-supply-chain-security-contract.test.mjs`，后端依赖同时运行 `timeout 60s mvn -f backend/pom.xml -pl mmmail-server -am -Dtest=DependencyVersionGuardTest -Dsurefire.failIfNoSpecifiedTests=false test`，前端依赖同时运行 `pnpm --dir frontend-admin typecheck` 和相关 build/test |
