...
import OidcSetupEntry from './modules/OidcSetupEntry.vue';

defineOptions({ name: 'Admin' });

const DEFAULT_BILLING_SEAT_COUNT = 1;
const CURRENCY_MINOR_UNIT = 100;
const ACCESSIBLE_STATISTIC_THEME = { labelTextColor: 'var(--n-text-color-2)' } as const;
...
const commercialLicenseStatus = ref<Api.Billing.CommercialLicenseStatus | null>(null);
...
async function loadCommercialLicenseStatus() {
  if (!orgStore.currentOrgId) {
    commercialLicenseStatus.value = null;
    return;
  }

  const { data, error } = await readCommercialLicenseStatus();
  if (!error) commercialLicenseStatus.value = data;
}

async function loadAdmin() {
  await Promise.all([loadOrgAdmin(), loadBilling(), loadCommercialLicenseStatus()]);
}
      ...
      <NGrid :x-gap="16" :y-gap="16" responsive="screen" item-responsive>
        ...
        <NGi span="24">
          <NCard class="card-wrapper" :title="$t('page.billing.title')">
            <NSpace vertical :size="16">
              <NGrid
                data-testid="billing-status-a11y-scope"
                :x-gap="16"
                :y-gap="16"
                responsive="screen"
                item-responsive
              >
                <NGi span="24 s:12 m:6">
                  <NStatistic
                    ...
                    :theme-overrides="ACCESSIBLE_STATISTIC_THEME"
                  />
                </NGi>
                <NGi span="24 s:12 m:6">
                  <NStatistic
                ...
                <NGi span="24 s:12 m:6">
                  <NStatistic
                    :label="$t('page.billing.externalBillingStatus')"
                    :value="commercialLicenseStatus?.externalBillingStatus || $t('common.noData')"
                    :theme-overrides="ACCESSIBLE_STATISTIC_THEME"
                  />
                </NGi>
                <NGi span="24 s:12 m:6">
                ...
                <NGi span="24 l:12">
                  <NForm :model="quoteModel" label-placement="top">
                    ...
                    <NSpace justify="end">
                      ...
                      <NButton type="primary" @click="submitCheckoutDraft">
                        {{ $t('page.billing.createDraft') }}
                      </NButton>
                    </NSpace>
                  </NForm>
                </NGi>
