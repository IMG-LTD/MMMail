...
function isLocked(rowKey: string, columnKey: string) {
  return props.protectedRanges.some(
    (item) => item.range.start.rowKey === rowKey && item.range.start.columnKey === columnKey,
  );
}
...
<template>
  ...
  >
    <div class="data-grid__scroll" role="grid" aria-label="Spreadsheet data grid">
      ...
      <div v-for="(row, rowIndex) in rows" :key="row.key" class="data-grid__row" role="row">
        ...
        <NButton
          ...
          :class="{
            'data-grid__cell--active': isActive(row.key, column.key),
            'data-grid__cell--locked': isLocked(row.key, column.key),
          }"
          role="gridcell"
          native-type="button"
    ...
    <p v-if="selectedRange" class="data-grid__range">
      Selected {{ selectedRange.start.rowKey }}:{{ selectedRange.start.columnKey }} to
      {{ selectedRange.end.rowKey }}:{{ selectedRange.end.columnKey }}
    </p>
  </section>
</template>
