...
import CommandPalette, { type CommandPaletteGroup, type CommandPaletteItem } from '@/design-system/components/CommandPalette.vue'
...
const commandGroups = computed<CommandPaletteGroup[]>(() => {
  return shellNavGroups.map((group, groupIndex) => ({
    ...
    items: group.items.map(item => ({
      ...
      description: group.caption ? tr(group.caption) : tr(lt('打开模块', '開啟模組', 'Open module'))
    }))
  }))
})
...
<template>
  <CommandPalette
    ...
    :placeholder="tr(lt('搜索邮件、文档、日程、联系人...', '搜尋郵件、文件、日程、聯絡人...', 'Search mail, docs, calendar, contacts...'))"
