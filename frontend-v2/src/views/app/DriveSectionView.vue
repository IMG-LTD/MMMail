...
import { V211ActionBar, V211DataToolbar } from "@/design-system/v211";
import {
  listDriveItems,
  listDriveShares,
  readDriveUsage,
...
const selectedItem = computed<DriveItem | null>(() => {
  return (
    visibleItems.value.find((item) => item.id === selectedItemId.value) ||
    visibleItems.value[0] ||
    null
  );
});
const driveToolbarFilters = [
  {
    id: "view",
    label: lt("视图", "視圖", "View"),
    options: [
      { label: lt("列表", "清單", "List"), value: "list" },
      { label: lt("网格", "網格", "Grid"), value: "grid" },
    ],
  },
  {
    id: "sort",
    label: lt("排序", "排序", "Sort"),
    options: [
      { label: lt("名称", "名稱", "Name"), value: "name" },
      { label: lt("最近修改", "最近修改", "Modified"), value: "updatedAt" },
    ],
  },
];
const driveBulkActions = [
  { id: "share", label: lt("共享", "共享", "Share") },
  { id: "download", label: lt("下载", "下載", "Download") },
];
const drivePrimaryAction = { id: "open-share", label: lt("管理共享", "管理共享", "Manage share") };
...
const usageCopy = computed(() => {
  if (!authStore.accessToken) {
    return tr(
      lt(
        "登录后即可读取文件数、文件夹数与云盘配额。",
        "登入後即可讀取檔案數、資料夾數與雲端硬碟配額。",
        "Sign in to load file, folder, and storage usage.",
      ),
    );
  }
  ...
  if (!driveUsage.value) {
    return tr(
      lt(
        "当前没有可显示的配额摘要。",
        "目前沒有可顯示的配額摘要。",
        "No usage summary is available yet.",
      ),
    );
  }
  ...
  if (!authStore.accessToken) {
    return tr(
      lt(
        "登录后即可读取云盘工作区。",
        "登入後即可讀取雲端硬碟工作區。",
        "Sign in to load your drive workspace.",
      ),
    );
  }
  ...
  if (!authStore.accessToken) {
    return tr(
      lt(
        "登录后即可加载文件详情与共享信息。",
        "登入後即可載入檔案詳情與共享資訊。",
        "Sign in to load file details and share data.",
      ),
    );
  }
  ...
  if (!selectedItem.value) {
    return tr(
      lt(
        "当前视图没有可检查的文件。",
        "目前檢視沒有可檢查的檔案。",
        "There is no file to inspect in this view.",
      ),
    );
  }
...
const shareEmptyCopy = computed(() => {
  if (!authStore.accessToken) {
    return tr(
      lt("登录后即可读取共享详情。", "登入後即可讀取共享詳情。", "Sign in to load share details."),
    );
  }
  ...
  if (!selectedItem.value) {
    return tr(
      lt(
        "选择一个项目以查看共享详情。",
        "選擇一個項目以查看共享詳情。",
        "Select an item to view share details.",
      ),
    );
  }

  return tr(
    lt("当前项目没有共享链接。", "目前項目沒有共享連結。", "This item has no share links."),
  );
});
    ...
    loadError.value = resolveRuntimeError(
      ...
      tr(
        lt(
          "读取云盘数据失败，请稍后重试。",
          "讀取雲端硬碟資料失敗，請稍後重試。",
          "Failed to load drive data. Please try again later.",
        ),
      ),
    );
  } finally {
...
async function loadDriveShares(
  itemId: string,
      ...
      resolveRuntimeError(
        ...
        tr(
          lt(
            "读取共享链接失败，请稍后重试。",
            "讀取共享連結失敗，請稍後重試。",
            "Failed to load share links. Please try again later.",
          ),
        ),
      );
  } finally {
...
<template>
  <section class="drive-surface">
    <aside class="drive-surface__nav">
      <NButton class="drive-surface__primary" native-type="button">{{
        tr(lt("+ 新建", "+ 新增", "+ New"))
      }}</NButton>
      <NButton
    ...
    <div class="drive-surface__content">
      <header class="drive-surface__head">
        ...
        <div class="drive-surface__actions">
          <V211DataToolbar
            :search-placeholder="lt('搜索文件', '搜尋檔案', 'Search files')"
            :filters="driveToolbarFilters"
            :bulk-actions="driveBulkActions"
            :create-label="lt('上传', '上傳', 'Upload')"
      ...
      <p v-if="!authStore.accessToken" class="page-subtitle drive-surface__status">
        {{
          tr(
            lt(
              "登录后即可读取文件列表、共享链接与配额信息。",
              "登入後即可讀取檔案清單、共享連結與配額資訊。",
              "Sign in to load drive items, share links, and usage data.",
            ),
          )
        }}
      </p>
      ...
      <article class="surface-card drive-surface__table">
        <V211ActionBar
          v-if="selectedItem"
          :selected-count="1"
          :primary-action="drivePrimaryAction"
          :secondary-actions="driveBulkActions"
          @action="openItemMenu(selectedItem.id)"
        />
        ...
        >
          ...
          <span>{{
            formatDateLabel(item.updatedAt, tr(lt("未知时间", "未知時間", "Unknown time")))
          }}</span>
        </NButton>
      ...
      <div class="drive-surface__cards">
        ...
        >
          <NButton
            class="drive-surface__card-main"
            native-type="button"
            @click="selectItem(item.id)"
          >
            ...
            <span class="page-subtitle">{{
              tr(
                lt(
                  `最后更新 ${formatDateLabel(item.updatedAt, tr(lt("未知时间", "未知時間", "Unknown time")))}。共享详情会在抽屉中加载。`,
                  `最後更新 ${formatDateLabel(item.updatedAt, tr(lt("未知时间", "未知時間", "Unknown time")))}。共享詳情會在抽屜中載入。`,
                  `Last updated ${formatDateLabel(item.updatedAt, tr(lt("未知时间", "未知時間", "Unknown time")))}. Share details load in the drawer.`,
                ),
              )
            }}</span>
          </NButton>
    ...
    <aside class="drive-surface__detail">
      ...
      <article class="surface-card drive-surface__panel">
        ...
        <NButton class="drive-share-trigger" native-type="button" @click="openSharePanel">
        ...
        <div v-if="driveShares.length" class="drive-surface__share-list">
          <div v-for="share in driveShares" :key="share.id" class="drive-surface__share-item">
            ...
            <span
              >{{ tr(lt("创建于", "建立於", "Created")) }}
              {{ formatDateTime(share.createdAt, tr(lt("未设置", "未設定", "Not set"))) }}</span
            >
          </div>
        </div>
