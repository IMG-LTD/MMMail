...
const auditItems: AuditItem[] = [
  ...
  },
  {
    ...
    meta: lt(
      "昨天 · 设备：iPhone 15 Pro",
      "昨天 · 裝置：iPhone 15 Pro",
      "Yesterday · Device: iPhone 15 Pro",
    ),
    tone: "info",
  },
  {
    ...
    meta: lt(
      "3 月 12 日 · 轮换已成功完成",
      "3 月 12 日 · 輪換已成功完成",
      "Mar 12 · Rotation completed successfully",
    ),
    tone: "muted",
  },
];
  ...
  <section class="page-shell surface-grid security-page">
    <compact-page-header
      ...
      :description="
        lt(
          '管理加密密钥、认证方式与活跃会话治理。',
          '管理加密金鑰、驗證方式與活躍工作階段治理。',
          'Manage encryption keys, authentication methods, and active session governance.',
        )
      "
    >
      <div class="security-page__actions">
        <NButton native-type="button">{{
          tr(lt("下载恢复包", "下載復原包", "Download Recovery Kit"))
        }}</NButton>
        <NButton native-type="button">{{
          tr(lt("查看会话", "查看工作階段", "Review Sessions"))
        }}</NButton>
      </div>
    </compact-page-header>
    ...
    <div class="security-cards">
      <article class="surface-card security-card">
        ...
        <div v-for="device in registeredDevices" :key="device.id" class="security-card__list-row">
          ...
          <NButton native-type="button">{{ tr(device.actionLabel) }}</NButton>
        </div>
      </article>
      ...
      <article class="surface-card security-card">
        <span class="section-label">{{
          tr(lt("加密基础设施", "加密基礎設施", "Encryption Infrastructure"))
        }}</span>
        <strong class="security-card__title">{{
          tr(lt("密钥健康状态最佳", "金鑰健康狀態最佳", "Key Health Optimal"))
        }}</strong>
        <p class="page-subtitle">
          {{
            tr(
              lt(
                "最近一次轮换：2 天前。你的加密身份已在可信端点间保持同步。",
                "最近一次輪換：2 天前。你的加密身分已在可信端點間保持同步。",
                "Last rotation: 2 days ago. Your cryptographic identity is synchronized across trusted endpoints.",
              ),
            )
          }}
        </p>
        <NButton class="security-card__link" native-type="button">{{
          tr(lt("查看技术详情", "查看技術詳情", "View Technical Details"))
        }}</NButton>
      </article>
        ...
        <strong class="security-card__title">{{
          tr(
            lt(
              "请保护好本地恢复记录。",
              "請保護好本機復原紀錄。",
              "Protect local recovery records.",
            ),
          )
        }}</strong>
        <p>
          {{
            tr(
              lt(
                "一旦丢失恢复包，历史加密数据可能永久锁定。MMMail 无法代你恢复。",
                "一旦遺失復原包，歷史加密資料可能永久鎖定。MMMail 無法代你復原。",
                "Losing your recovery kit means historical encrypted data may stay locked. MMMail cannot recover this for you.",
              ),
            )
          }}
        </p>
        <div class="security-card__dark-actions">
          <NButton native-type="button">{{ tr(lt("下载", "下載", "Download")) }}</NButton>
          <NButton native-type="button">{{
            tr(lt("轮换恢复包", "輪換復原包", "Rotate Kit"))
          }}</NButton>
        </div>
      </article>
    </div>
    ...
    <article class="surface-card security-audit">
      <div class="security-audit__head">
        <span class="section-label">{{
          tr(lt("安全活动审计", "安全活動稽核", "Security Activity Audit"))
        }}</span>
        <div class="security-audit__filters">
          <NButton native-type="button">{{ tr(lt("全部", "全部", "All")) }}</NButton>
          <NButton native-type="button">{{
            tr(lt("高严重度", "高嚴重度", "High Severity"))
          }}</NButton>
        </div>
      </div>
    ...
    <article class="surface-card security-sessions">
      <div class="security-sessions__head">
        <span class="section-label">{{
          tr(lt("活跃会话治理", "活躍工作階段治理", "Active Sessions Governance"))
        }}</span>
        <NButton native-type="button">{{
          tr(lt("刷新列表", "重新整理清單", "Refresh List"))
        }}</NButton>
      </div>
