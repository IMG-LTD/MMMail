...
export function isProtectedRangeCell(rowIndex: number, colIndex: number) {
  return rowIndex >= PROTECTED_RANGE_START_ROW_INDEX
...
export function updateGridCell(grid: string[][], rowIndex: number, colIndex: number, value: string) {
...
export function resolveSheetGrid(sheet: SheetsWorkbookSheet | null, workbook: SheetsWorkbookDetail | null) {
...
export function buildPendingEdits(sheet: SheetsWorkbookSheet | null, workbook: SheetsWorkbookDetail | null, grid: string[][]) {
  if (!sheet) {
    return []
  }
