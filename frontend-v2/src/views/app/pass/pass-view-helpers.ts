...
export function compareItemsForVault(left: PassWorkspaceItemSummary, right: PassWorkspaceItemSummary) {
  return Number(right.favorite) - Number(left.favorite) || compareItemsByUpdated(left, right)
}

export function compareItemsForSecureLinks(left: PassWorkspaceItemSummary, right: PassWorkspaceItemSummary) {
...
export function compareMonitorItems(left: PassMonitorItem, right: PassMonitorItem) {
  return countMonitorSignals(right) - countMonitorSignals(left) || compareDateDesc(left.updatedAt, right.updatedAt)
}

export function compareMailboxes(left: PassMailbox, right: PassMailbox) {
  return Number(right.defaultMailbox) - Number(left.defaultMailbox)
...
export function compareItemsByUpdated(left: Pick<PassWorkspaceItemSummary, 'title' | 'updatedAt'>, right: Pick<PassWorkspaceItemSummary, 'title' | 'updatedAt'>) {
...
export function resolvePassItemType(itemType: PassWorkspaceItemSummary['itemType'] | PassMonitorItem['itemType']) {
  const names = { ALIAS: 'Alias', CARD: 'Card', LOGIN: 'Login', NOTE: 'Secure note', PASSKEY: 'Passkey', PASSWORD: 'Password' }
  return names[itemType] || itemType
}

export function resolvePassScope(scopeType: PassWorkspaceItemSummary['scopeType'] | PassMonitorItem['scopeType']) {
