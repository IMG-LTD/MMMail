...
export function resolveCalendarTimeSlotHours(
  items: Array<{ endAt: string; startAt: string }>,
  fallbackHour: number,
) {
  ...
  return createHourRange(
    Math.max(MIN_HOUR, Math.min(...hours) - 1),
    Math.min(MAX_HOUR, Math.max(...hours) + 1),
  );
}
...
export function resolvePositionedCalendarEvents(
  ...
  return items.flatMap((item) =>
    createVisibleSegments(item, options.days).map((segment) =>
      toPositionedEvent(item, segment, options),
    ),
  );
}
...
function createVisibleSegments(
  item: CalendarLayoutEvent,
  days: CalendarLayoutDay[],
): CalendarSegment[] {
  const startAt = parseDate(item.startAt);
  const endAt = parseDate(item.endAt);
...
function toPositionedEvent(
  ...
  const rowStart = Math.max(
    FIRST_EVENT_ROW,
    Math.floor(hourFraction(segment.startAt) - firstSlotHour) + FIRST_EVENT_ROW,
  );
  const rowSpan = Math.max(
    1,
    Math.ceil(resolveSegmentEndHour(segment) - hourFraction(segment.startAt)),
  );
  return {
    id: item.id,
    key: segment.key,
    ...
    style: {
      gridColumn: String(dayIndex + FIRST_DAY_COLUMN),
...
function resolveSegmentEndHour(segment: { endAt: Date; startAt: Date }) {
  return formatDateKey(segment.endAt) === formatDateKey(segment.startAt)
    ? hourFraction(segment.endAt)
    : HOURS_PER_DAY;
}
