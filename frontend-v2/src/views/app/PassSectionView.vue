...
const sharedItems = computed(() => passItems.value.filter(isSharedItem).slice().sort(compareItemsForVault))
const personalItems = computed(() => passItems.value.filter(item => !isPassItemShared(item)).slice().sort(compareItemsForVault))
const secureLinkItems = computed(() => passItems.value.filter(item => item.secureLinkCount > 0).slice().sort(compareItemsForSecureLinks))
const aliasItems = computed(() => passItems.value.filter(item => item.itemType === 'ALIAS').slice().sort(compareItemsForVault))
const monitorIssueMap = computed(() => createPassMonitorIssueMap(passMonitor.value))
const policyItems = computed(() => Array.from(monitorIssueMap.value.values()).sort(compareMonitorItems))
const derivedState = computed(() => derivePassSectionState(passItems.value, passMailboxes.value, passMonitor.value, PASS_ITEMS_FETCH_LIMIT))
const visibleEntries = computed<PassSurfaceEntry[]>(resolveVisibleEntries)
const selectedEntry = computed(() => visibleEntries.value.find(entry => entry.key === selectedEntryKey.value) || visibleEntries.value[0] || null)
const selectedItem = computed(() => selectedEntry.value?.item || findItemForMonitorEntry(selectedEntry.value) || null)
const selectedMailbox = computed(() => selectedEntry.value?.mailbox || null)
const selectedMonitorItem = computed(() => selectedEntry.value?.monitorItem || monitorIssueMap.value.get(selectedItem.value?.id || '') || null)
const activeRisk = computed(() => policyItems.value.find(item => item.id === selectedRiskId.value) || selectedMonitorItem.value || policyItems.value[0] || null)
...
function isCurrentPassRequest(requestId: number, requestToken: string, requestPath: string) {
  return requestId === latestPassRequest && requestToken === authStore.accessToken && requestPath === route.fullPath
}
...
function resolveVisibleEntries() {
  if (surfaceKey.value === 'pass-mailbox') return passMailboxes.value.slice().sort(compareMailboxes).map(createMailboxEntry)
  if (surfaceKey.value === 'pass-shared-library') return sharedItems.value.map(item => createItemEntry(item, surfaceKey.value))
  if (surfaceKey.value === 'pass-secure-links') return secureLinkItems.value.map(item => createItemEntry(item, surfaceKey.value))
  if (surfaceKey.value === 'pass-alias-center') return aliasItems.value.map(item => createItemEntry(item, surfaceKey.value))
  if (surfaceKey.value === 'pass-business-policy' || surfaceKey.value === 'pass-monitor') return policyItems.value.map(createMonitorEntry)
...
function createItemEntry(item: PassWorkspaceItemSummary, key: string): PassSurfaceEntry {
  ...
  return {
    ...
    badge: key === 'pass-secure-links' ? `${item.secureLinkCount} links` : item.favorite ? 'Starred' : resolvePassItemType(item.itemType),
    id: item.id,
    item,
    key: `item:${item.id}`,
    meta: joinText([resolvePassScope(item.scopeType), item.sharedVaultId ? 'Shared vault' : null, formatDateTime(item.updatedAt)]),
...
function createMailboxEntry(mailbox: PassMailbox): PassSurfaceEntry {
  return {
    ...
    badge: mailbox.defaultMailbox ? 'Default' : mailbox.primaryMailbox ? 'Primary' : resolveMailboxStatus(mailbox),
    id: mailbox.id,
    key: `mailbox:${mailbox.id}`,
    mailbox,
    meta: joinText([mailbox.defaultMailbox ? 'Default routing' : null, mailbox.primaryMailbox ? 'Primary mailbox' : null, formatDateTime(mailbox.verifiedAt || mailbox.updatedAt)]),
...
function findItemForMonitorEntry(entry: PassSurfaceEntry | null) {
  return entry?.monitorItem ? passItems.value.find(item => item.id === entry.monitorItem?.id) || null : null
}

function createPrimaryCard() {
  if (!authStore.accessToken) return createCard('Authenticated runtime', 'Waiting for sign-in', 'Sign in to load pass items, mailboxes, and monitor summary.')
  if (surfaceKey.value === 'pass-mailbox') return createMailboxCard(selectedMailbox.value)
  if (surfaceKey.value === 'pass-business-policy' || surfaceKey.value === 'pass-monitor') return createPolicyCard(selectedMonitorItem.value)
...
function createItemCard(item: PassWorkspaceItemSummary | null, monitorItem: PassMonitorItem | null) {
  if (!item) return createCard('Selected item', 'No item selected', 'Select an item from the list to inspect authenticated detail.')
  return createCard('Selected item', item.title || 'Untitled item', joinText([item.website, item.username, monitorItem ? resolveMonitorFlags(monitorItem) : null]), [
...
function createMailboxCard(mailbox: PassMailbox | null) {
  if (!mailbox) return createCard('Selected mailbox', 'No mailbox selected', 'Select a mailbox to inspect verification state.')
...
function createPolicyCard(item: PassMonitorItem | null) {
  if (!item) return createCard('Policy focus', 'No item selected', 'Select a monitored item to inspect risk state.')
...
function createCard(label: string, title: string, copy: string, facts: PassDetailCard['facts'] = []): PassDetailCard {
...
function resolveErrorMessage(error: unknown) {
  return error instanceof Error && error.message ? error.message : 'Failed to load pass data. Please try again later.'
}
...
watch(visibleEntries, (entries) => {
  if (!entries.some(entry => entry.key === selectedEntryKey.value)) selectedEntryKey.value = entries[0]?.key || ''
