...
const current = computed(() =>
  findSurface(labsModules, String(route.params.moduleKey ?? "authenticator"), "authenticator"),
);
...
const description = computed(
  () => moduleDetail.value?.description || tr(current.value.description),
);
  ...
  if (!authStore.accessToken) {
    return tr(
      lt(
        "登录后即可读取模块配置。",
        "登入後即可讀取模組設定。",
        "Sign in to load module configuration.",
      ),
    );
  }
  ...
  return moduleLoading.value
    ...
    : tr(
        lt(
          "模块配置来自 v2.1 运行时。",
          "模組設定來自 v2.1 執行期。",
          "Module configuration is loaded from the v2.1 runtime.",
        ),
      );
});
...
async function loadLabsModule() {
  ...
  try {
    ...
    if (
      requestId !== latestLabsModuleRequest ||
      requestToken !== authStore.accessToken ||
      requestModuleKey !== moduleKey.value
    ) {
      return;
    }
...
async function saveModuleSettings() {
  ...
  await patchLabsModuleSettings(
    moduleKey.value,
    { enabled: !moduleDetail.value.enabled },
    requestToken,
  );
...
watch(
  () => [moduleKey.value, authStore.accessToken],
  () => {
    void loadLabsModule();
  },
  { immediate: true },
  ...
  <section class="page-shell surface-grid labs-module">
    <article class="surface-card labs-module__hero">
      ...
      <div class="labs-module__actions">
        <span class="metric-chip">{{
          moduleDetail?.maturity || tr(current.badge ?? lt("预览", "預覽", "Preview"))
        }}</span>
        <span class="metric-chip">{{
          moduleDetail?.enabled
            ? tr(lt("已启用", "已啟用", "Enabled"))
            : tr(lt("未启用", "未啟用", "Disabled"))
        }}</span>
      </div>
    </article>
    <article class="surface-card labs-module__panel">
      ...
      <p class="page-subtitle">
        {{
          moduleDetail
            ? `${moduleDetail.premium ? "Premium" : "Community"} · ${moduleDetail.hosted ? "Hosted optional" : "Self-hosted"}`
            : moduleStatusCopy
        }}
      </p>
      <NButton
        native-type="button"
        :disabled="!moduleDetail || settingsSaving"
        @click="saveModuleSettings()"
      >
        {{
          settingsSaving
            ? tr(lt("保存中", "儲存中", "Saving"))
            : tr(lt("切换启用状态", "切換啟用狀態", "Toggle enabled"))
        }}
      </NButton>
    </article>
  </section>
