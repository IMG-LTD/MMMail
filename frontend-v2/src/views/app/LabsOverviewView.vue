...
const statusCopy = computed(() => {
  if (!authStore.accessToken) {
    return tr(
      lt("登录后即可读取实验模块。", "登入後即可讀取實驗模組。", "Sign in to load Labs modules."),
    );
  }
...
<template>
  <section class="page-shell surface-grid">
    <compact-page-header
      ...
      :description="
        lt(
          '实验能力可以被看见，但必须在视觉上从属于主功能，并明确标记为不稳定。',
          '實驗能力可以被看見，但必須在視覺上從屬於主要功能，並明確標記為不穩定。',
          'Experimental capabilities are visible, but they stay visually subordinate and clearly marked as unstable.',
        )
      "
      ...
      <strong>{{
        tr(
          lt(
            "实验能力不保证稳定性或兼容性。",
            "實驗能力不保證穩定性或相容性。",
            "Experimental capabilities do not guarantee stability or compatibility.",
          ),
        )
      }}</strong>
    ...
    <div class="labs-grid">
      <article class="surface-card labs-card">
        ...
        <div class="labs-card__stack">
          <span v-for="item in enabledModules" :key="item.key" class="metric-chip">{{
            item.label
          }}</span>
      ...
      <article class="surface-card labs-card">
        ...
        <div class="labs-card__stack labs-card__stack--muted">
          <span v-for="item in disabledModules" :key="item.key" class="metric-chip">{{
            item.label
          }}</span>
