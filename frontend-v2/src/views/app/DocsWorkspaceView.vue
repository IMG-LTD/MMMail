  ...
  if (!authStore.accessToken) {
    return tr(
      lt(
        "登录后即可读取文档工作区。",
        "登入後即可讀取文件工作區。",
        "Sign in to load your docs workspace.",
      ),
    );
  }
...
const listEmptyCopy = computed(() => {
  if (!authStore.accessToken) {
    return tr(
      lt(
        "登录后即可查看最近、个人和共享文档。",
        "登入後即可查看最近、個人與共享文件。",
        "Sign in to view recent, personal, and shared docs.",
      ),
    );
  }
  ...
  return tr(
    lt("当前没有可显示的文档。", "目前沒有可顯示的文件。", "No docs are available right now."),
  );
});
...
async function loadDocsWorkspace() {
  ...
  if (!requestToken) {
    if (
      requestId !== latestDocsWorkspaceRequest ||
    ...
    loadError.value = resolveErrorMessage(
      ...
      tr(
        lt(
          "读取文档列表失败，请稍后重试。",
          "讀取文件清單失敗，請稍後重試。",
          "Failed to load docs. Please try again later.",
        ),
      ),
    );
  } finally {
...
<template>
  <section class="page-shell surface-grid">
    <compact-page-header
      ...
      :description="
        lt(
          '面向最近、共享和个人写作的 Beta 界面，但不承诺完整套件对等能力。',
          '面向最近、共享與個人寫作的 Beta 介面，但不承諾完整套件對等能力。',
          'Beta surfaces for recent, shared, and personal writing without promising full-suite parity.',
        )
      "
