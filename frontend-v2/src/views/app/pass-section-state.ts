...
export function derivePassSectionState(
  ...
  const defaultMailbox =
    mailboxes.find((mailbox) => mailbox.defaultMailbox) || mailboxes[0] || null;
  const primaryMailbox =
    mailboxes.find((mailbox) => mailbox.primaryMailbox) || mailboxes[0] || null;
  ...
  return {
    ...
    sharedVaultCount: new Set(
      sharedItems
        .map((item) => item.sharedVaultId)
