...
import {
  resolveCalendarTimeSlotHours,
  resolvePositionedCalendarEvents,
} from "./calendar/calendar-layout";
import {
  addDays,
  formatDateKey,
  formatDateWindow,
...
const surfaceItems = computed(() =>
  mergeCalendarSurfaceItems(calendarEvents.value, agendaItems.value),
);
...
const toolbarMonthLabel = computed(() =>
  (scheduleDays.value[0]?.date || focusedDay.value).toLocaleDateString(undefined, {
    month: "long",
    year: "numeric",
  }),
);
...
const selectedItem = computed<CalendarSurfaceItem | null>(
  () =>
    surfaceItems.value.find((item) => item.id === selectedEventId.value) ||
    surfaceItems.value[0] ||
    null,
);
const boardSubtitle = computed(() =>
  authStore.accessToken
    ? `Events ${calendarEvents.value.length} · Agenda ${agendaItems.value.length}`
    : "Sign in to switch this calendar surface to runtime data.",
);
const emptyState = computed(() =>
  authStore.accessToken
    ? "No events in the current time window."
    : "Sign in to load your calendar workspace.",
);
const visibleRange = computed(() => {
  const rangeStart =
    activeViewMode.value === "day" ? startOfDay(focusedDay.value) : startOfWeek(focusedDay.value);
  return {
    from: formatLocalDateTime(rangeStart),
    to: formatLocalDateTime(addDays(rangeStart, activeViewMode.value === "day" ? 1 : 5)),
  };
});
...
function isCurrentCalendarRequest(request: {
  id: number;
  ...
  return (
    request.id === latestCalendarRequest &&
    request.token === authStore.accessToken &&
    rangeKey === `${visibleRange.value.from}|${visibleRange.value.to}`
  );
}

function resetSignedOutCalendar(request: {
  id: number;
  range: { from: string; to: string };
  token: string;
}) {
...
async function applyCalendarResponses(
  request: { id: number; range: { from: string; to: string }; token: string },
  ...
  loadError.value = "";
  selectedEventId.value =
    surfaceItems.value.find((item) => item.id === selectedEventId.value)?.id ||
    surfaceItems.value[0]?.id ||
    "";
}
...
async function readAvailability(
  ...
  try {
    const response = await queryCalendarAvailability(request.token, {
      attendeeEmails,
      endAt: request.range.to,
      startAt: request.range.from,
    });
...
function createScheduleDays() {
  const baseDate =
    activeViewMode.value === "day" ? startOfDay(focusedDay.value) : startOfWeek(focusedDay.value);
  return Array.from({ length: workWeekLength(activeViewMode.value) }, (_, index) =>
    createDayCell(addDays(baseDate, index)),
  );
}
...
function createDayCell(date: Date): CalendarDayCell {
  ...
  return {
    ...
    hasEvents: surfaceItems.value.some(
      (item) => formatDateKey(parseDate(item.startAt) || date) === key,
    ),
    key,
...
function resolveToolbarRangeLabel() {
  ...
  if (activeViewMode.value === "day")
    return first.toLocaleDateString(undefined, { weekday: "long", month: "long", day: "numeric" });
...
function resolveCalendarSummaries() {
  ...
  const ownedCount = surfaceItems.value.filter(
    (item) => !item.ownerEmail || item.ownerEmail === currentUserEmail,
  ).length;
  ...
  return Array.from(
    new Set(
      [
        ...
        ...events.map((item) => item.ownerEmail || ""),
        ...agenda.map((item) => item.ownerEmail || ""),
      ].filter(isEmailLike),
    ),
  );
}
