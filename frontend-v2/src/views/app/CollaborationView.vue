  ...
  if (!authStore.accessToken) {
    return tr(
      lt(
        "登录后即可读取协作项目。",
        "登入後即可讀取協作專案。",
        "Sign in to load collaboration projects.",
      ),
    );
  }
...
const summaryCards = computed(() => [
  ...
  [
    ...
    `${tasks.value.length}`,
    tr(
      lt("来自当前组织范围。", "來自目前組織範圍。", "Loaded from the current organization scope."),
    ),
  ],
  [
    lt("活动", "活動", "Activity"),
    `${activity.value.length}`,
    tr(lt("最近协作事件。", "最近協作事件。", "Recent collaboration events.")),
  ],
]);
...
<template>
  <section class="page-shell surface-grid">
    <compact-page-header
      ...
      :description="
        lt(
          '跨模块协作信号会按当前范围内可见产品进行过滤。',
          '跨模組協作訊號會依目前範圍內可見產品進行過濾。',
          'Cross-module collaboration signals filtered to the products visible in the current scope.',
        )
      "
    ...
    <div class="collaboration-filters">
      <span class="metric-chip">{{
        tr(lt("当前筛选：共享工作", "目前篩選：共享工作", "Current filter: Shared work"))
      }}</span>
      <span class="metric-chip">{{
        tr(lt("感知组织访问范围", "感知組織存取範圍", "Org access aware"))
      }}</span>
      <span class="metric-chip">{{
        tr(lt("不含邮件的摘要", "不含郵件的摘要", "Mail-free summaries"))
      }}</span>
      <span v-for="project in projects" :key="project.id" class="metric-chip">{{
        project.name
      }}</span>
    </div>
    ...
    <div class="collaboration-grid">
      <article
        v-for="([title, value, copy], index) in summaryCards"
        :key="index"
        class="surface-card collaboration-card"
      >
