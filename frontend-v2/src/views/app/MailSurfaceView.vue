...
import { V211DataToolbar } from "@/design-system/v211";
import {
  listMailFolder,
...
import {
  ...
  mailFolderSurfaces,
  type SurfaceOption,
} from "@/shared/content/route-surfaces";
...
const localNav = computed<SurfaceOption[]>(() => [
  ...
  {
    key: "contacts",
    label: lt("联系人", "聯絡人", "Contacts"),
    description: lt(
      "发件身份与可信通讯地址。",
      "寄件身分與可信通訊地址。",
      "Sender identities and trusted contact addresses.",
    ),
  },
  {
    key: "search",
    label: lt("搜索", "搜尋", "Search"),
    description: lt(
      "按关键字读取已认证邮件结果。",
      "依關鍵字讀取已驗證郵件結果。",
      "Read authenticated mail results by keyword.",
    ),
  },
  {
    key: "compose",
    label: lt("写邮件", "寫郵件", "Compose"),
    description: lt(
      "基于已认证身份与收件人信任状态写信。",
      "基於已驗證身分與收件者信任狀態寫信。",
      "Compose with authenticated identities and recipient trust state.",
    ),
  },
]);
...
const currentFolder = computed(() =>
  isConversation.value ? "inbox" : isSearch.value ? "search" : surfaceKey.value,
);
...
const mailToolbarFilters = [
  {
    id: "state",
    label: lt("状态", "狀態", "State"),
    options: [
      { label: lt("全部", "全部", "All"), value: "all" },
      { label: lt("未读", "未讀", "Unread"), value: "unread" },
    ],
  },
];
const mailBulkActions = [
  { id: "archive", label: lt("归档", "封存", "Archive") },
  { id: "mark-read", label: lt("标为已读", "標示為已讀", "Mark read") },
];
...
function isCurrentWorkspaceRequest(request: { id: number; path: string; token: string }) {
  return (
    request.id === latestWorkspaceRequest &&
    request.token === authStore.accessToken &&
...
async function loadFolderForRequest(request: { id: number; path: string; token: string }) {
  ...
  try {
    ...
    if (isCurrentWorkspaceRequest(request))
      handleWorkspaceError(error, "Unable to load the mail list.");
    return [];
  }
}
...
async function loadDetailForRequest(
  request: { id: number; path: string; token: string },
  items: MailSummary[],
) {
...
function resolveDetailId(items: MailSummary[]) {
  return (
    resolveRouteString(route.params.threadId) ||
    resolveRouteString(route.params.id) ||
    items[0]?.id ||
    ""
  );
}
...
async function refreshRecipientTrust() {
  ...
  if (!requestToken || !isEmailLike(toEmail) || !isEmailLike(fromEmail))
    return resetRecipientTrust(requestId, requestToken);
  ...
  try {
    ...
    if (requestId === latestRecipientTrustRequest)
      recipientTrust.value = {
        status: "warning",
        message: "The recipient trust state is temporarily unavailable.",
      };
  } finally {
...
async function sendComposePayload() {
  ...
  try {
    ...
    composeSendError.value =
      error instanceof Error && error.message ? error.message : "Failed to send the message.";
  } finally {
  ...
  if (!authStore.accessToken)
    return tr(
      lt(
        "登录后即可读取已认证邮件详情。",
        "登入後即可讀取已驗證郵件詳情。",
        "Sign in to read authenticated mail detail.",
      ),
    );
  if (workspaceLoading.value)
    return tr(
      lt(
        "正在读取已认证邮件数据。",
        "正在讀取已驗證郵件資料。",
        "Loading authenticated mail data.",
      ),
    );
  if (activeMail.value)
    return tr(
      lt(
        "此消息详情来自已认证运行时数据。",
        "此訊息詳情來自已驗證執行期資料。",
        "This message detail comes from authenticated runtime data.",
      ),
    );
  return tr(
    lt("选择一封邮件以查看详情。", "選擇一封郵件以查看詳情。", "Select a message to view detail."),
  );
}
  ...
  if (!authStore.accessToken)
    return tr(
      lt(
        "登录后即可校验收件人信任状态。",
        "登入後即可驗證收件者信任狀態。",
        "Sign in to verify recipient trust state.",
      ),
    );
  if (recipientTrustLoading.value)
    return tr(
      lt(
        "正在检查收件人的加密投递状态。",
        "正在檢查收件者的加密投遞狀態。",
        "Checking the recipient encrypted-delivery status.",
      ),
    );
  return (
    recipientTrust.value?.message ||
    tr(
      lt(
        "填写发件人与收件人后，系统会读取信任状态。",
        "填寫寄件人與收件者後，系統會讀取信任狀態。",
        "Enter sender and recipient details to load the trust state.",
      ),
    )
  );
}
...
watch(
  () => [composeForm.value.toEmail, composeForm.value.fromEmail, authStore.accessToken],
  () => {
  ...
  >
    <header class="mail-surface__toolbar">
      ...
      <div class="mail-surface__toolbar-right">
        <V211DataToolbar
          :search-placeholder="lt('搜索邮件', '搜尋郵件', 'Search mail')"
          :filters="mailToolbarFilters"
          :bulk-actions="mailBulkActions"
          :selected-count="activeMail ? 1 : 0"
          @search="
            (value) => {
              searchDraft = value;
            }
          "
        />
        <NButton native-type="button" @click="copilotPanel.toggle()">{{
          copilotOpen ? "Copilot open" : "Toggle Copilot"
        }}</NButton>
      </div>
    </header>
    <p v-if="!authStore.accessToken" class="mail-surface__notice page-subtitle">
      Sign in to load mail, sender identities, and recipient trust state.
    </p>
    <p v-else-if="loadError && !isCompose" class="mail-surface__notice page-subtitle">
      {{ loadError }}
    </p>
    ...
    <section v-else-if="isContacts" class="mail-directory">
      <article
        ...
        class="surface-card mail-directory__card"
      >
        <strong>{{ resolveIdentityLabel(identity) }}</strong>
        <p>{{ identity.emailAddress }}</p>
        <NButton native-type="button" @click="openComposeForIdentity(identity.emailAddress)"
          >Compose with this identity</NButton
        >
      </article>
      <p v-if="!senderOptions.length" class="page-subtitle">
        Sender identities appear after sign-in.
      </p>
    </section>
    <section v-else-if="isSearch" class="mail-search">
      <div class="mail-search__head">
        <NInput
          ...
          placeholder="Search by subject or body keyword"
          @keyup.enter="runSearch"
        />
        <NButton native-type="button" @click="runSearch">Search</NButton>
      </div>
      <MailMessageList
        :empty-copy="'No matching mail found.'"
    ...
    <section v-else class="mail-workspace">
      ...
      <MailMessageList
        v-if="!isConversation"
        :empty-copy="'This folder is empty.'"
        :loading="workspaceLoading"
        :messages="mailItems"
        @open="openConversation"
      />
      <MailThreadReader
        :active-mail="activeMail"
        :loading="workspaceLoading"
        :show-back="isConversation"
        :trust-copy="detailTrustCopy"
        @back="router.push('/mail/inbox')"
      />
    </section>
  </section>
</template>
