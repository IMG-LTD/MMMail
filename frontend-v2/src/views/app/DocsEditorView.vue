...
const editorStatus = computed(() => {
  if (!authStore.accessToken) {
    return tr(
      lt(
        "登录后即可读取文档详情。",
        "登入後即可讀取文件詳情。",
        "Sign in to load document detail.",
      ),
    );
  }
  ...
  return tr(
    lt(
      "文档已从运行时接口载入。",
      "文件已從執行期介面載入。",
      "Document loaded from runtime APIs.",
    ),
  );
});
...
async function loadNote() {
  ...
  if (!requestToken || !requestNoteId) {
    if (
      requestId !== latestDocsNoteRequest ||
  ...
  try {
    ...
    if (
      ...
      requestToken !== authStore.accessToken ||
      requestPath !== route.fullPath ||
      requestNoteId !== String(route.params.id || "")
    ) {
      return;
    }
...
async function saveNote() {
  ...
  try {
    const response = await updateDocsNote(
      requestNoteId,
      {
    ...
    if (
      requestId !== latestDocsSaveRequest ||
      requestToken !== authStore.accessToken ||
      requestPath !== route.fullPath ||
      requestNoteId !== String(route.params.id || "")
    ) {
    ...
    saveError.value = resolveErrorMessage(
      ...
      tr(
        lt(
          "保存文档失败，请稍后重试。",
          "儲存文件失敗，請稍後重試。",
          "Failed to save the document. Please try again later.",
        ),
      ),
    );
  } finally {
...
watch(
  () => [noteId.value, route.fullPath, authStore.accessToken],
  ...
  <section class="page-shell surface-grid docs-editor">
    <article class="surface-card docs-editor__top">
      <div>
        ...
        <p class="page-subtitle">
          {{
            tr(
              lt(
                "块级编辑、提纲导航、协作者上下文和 Beta 指引都保持可见，但不夸大能力对等。",
                "區塊編輯、大綱導覽、協作者內容脈絡與 Beta 指引都保持可見，但不誇大能力對等。",
                "Block editing, outline navigation, collaborator context, and beta guidance remain visible without overstating parity.",
              ),
            )
          }}
        </p>
      ...
      <div class="docs-editor__actions">
        <NButton class="docs-share-trigger" native-type="button" @click="docsSharePanelOpen = true">
          {{ shareLabel }}
        </NButton>
        <NButton native-type="button">{{ note ? `v${note.currentVersion}` : "v0" }}</NButton>
        <NButton native-type="button" :disabled="saveDisabled" @click="saveNote()">
    ...
    <section class="docs-editor__layout">
      <aside class="surface-card docs-editor__panel">
        ...
        <NButton v-for="item in outlineItems" :key="item.key" native-type="button">{{
          item.label
        }}</NButton>
        <p v-if="!outlineItems.length" class="page-subtitle docs-editor__empty">
          {{
            tr(
              lt(
                "输入内容后，提纲会根据运行时文档生成。",
                "輸入內容後，大綱會依據執行期文件產生。",
                "The outline is generated from the runtime document content.",
              ),
            )
          }}
        </p>
      ...
      <aside class="surface-card docs-editor__panel">
        <span class="section-label">{{
          tr(lt("运行时详情", "執行期詳情", "Runtime detail"))
        }}</span>
        <strong>{{ collaborationHeading }}</strong>
        <p class="page-subtitle">
          {{
            note
              ? `${note.ownerEmail} · ${formatDateTime(note.updatedAt)}`
              : tr(
                  lt(
                    "认证后会在此显示文档状态。",
                    "驗證後會在此顯示文件狀態。",
                    "Document state appears here after authentication.",
                  ),
                )
          }}
        </p>
