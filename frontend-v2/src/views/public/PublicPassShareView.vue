  ...
  <section class="public-shell page-shell share-pass">
    <article class="surface-card share-pass__auth">
      <span class="section-label">{{
        tr(lt("受保护访问", "受保護存取", "Protected access"))
      }}</span>
      <h1 class="page-title">
        {{ share?.title || tr(lt("打开安全链接", "開啟安全連結", "Open a secure link")) }}
      </h1>
        ...
        {{
          ...
          tr(
            lt(
              "该链接会按路由令牌加载共享秘密项。",
              "此連結會依路由權杖載入共享秘密項目。",
              "This link loads the shared secret item from the route token.",
            ),
          )
        }}
      </p>
    ...
    <article class="surface-card share-pass__preview">
      ...
      <template v-if="share">
        ...
        <p class="page-subtitle">
          ...
          {{ tr(lt("查看次数", "檢視次數", "Views")) }}: {{ share.currentViews }}/{{
            share.maxViews
          }}
        ...
        <div class="share-pass__meta">
          <div class="metric-chip">
            {{ tr(lt("用户名", "使用者名稱", "Username")) }}: {{ displayValue(share.username) }}
          </div>
          <div class="metric-chip">
            {{ tr(lt("网站", "網站", "Website")) }}: {{ displayValue(share.website) }}
          </div>
        </div>
        ...
        <div class="share-pass__cards">
          <div class="share-pass__card">
            ...
            <n-button
              ...
              @click="copySecret(share.secretCiphertext, tr(lt('秘密', '秘密', 'Secret')))"
            >
          ...
          <div class="share-pass__card">
            ...
            <n-button
              ...
              @click="copySecret(share.username, tr(lt('用户名', '使用者名稱', 'Username')))"
            >
      ...
      <p v-else class="page-subtitle">
        {{
          loading
            ? tr(lt("正在加载共享条目。", "正在載入共享項目。", "Loading shared item."))
            : tr(lt("共享条目不可用。", "共享項目不可用。", "Shared item unavailable."))
        }}
      </p>
    </article>
  </section>
