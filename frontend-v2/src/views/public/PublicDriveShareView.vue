...
async function downloadItem(item: DriveShareItem) {
  ...
  try {
    const payload = await downloadPublicDriveShareItem(
      token.value,
      item.id,
  ...
  <section class="public-shell page-shell share-drive">
    <article class="surface-card share-drive__hero">
      <span class="section-label">{{
        tr(lt("安全共享访问", "安全共享存取", "Secure share access"))
      }}</span>
      <h1 class="page-title">
        {{
          metadata?.rootItemName ||
          tr(lt("共享文件夹访问", "共享資料夾存取", "Shared folder access"))
        }}
      </h1>
      <p class="page-subtitle">
        {{
          ...
          tr(
            lt(
              "该链接会按路由令牌加载共享元数据和文件列表。",
              "此連結會依路由權杖載入共享中繼資料與檔案清單。",
              "This link loads share metadata and item listings from the route token.",
            ),
          )
        }}
      </p>
      ...
      <div v-if="metadata" class="share-drive__meta">
        <div class="metric-chip">
          {{ tr(lt("权限", "權限", "Permission")) }}: {{ metadata.permission }}
        </div>
        <div class="metric-chip">
          {{ tr(lt("项目数", "項目數", "Items")) }}: {{ metadata.itemCount }}
        </div>
        <div class="metric-chip">
          {{ tr(lt("到期", "到期", "Expires")) }}: {{ formatDateTime(metadata.expiresAt) }}
        </div>
      </div>
        ...
        :placeholder="
          tr(
            lt(
              '输入共享密码以刷新列表',
              '輸入共享密碼以重新整理清單',
              'Enter the share password to refresh the list',
            ),
          )
        "
      />
    ...
    <article class="surface-card share-drive__items">
        ...
        <div v-for="item in items" :key="item.id" class="share-drive__item">
          <div>
            ...
            <span>
              ...
              <template v-if="typeof item.size === 'number'">
                · {{ formatBytes(item.size) }}</template
              >
            </span>
          </div>
      ...
      <p v-else class="page-subtitle">
        {{
          loading
            ? tr(lt("正在加载项目列表。", "正在載入項目清單。", "Loading shared items."))
            : tr(
                lt(
                  "没有可显示的共享项目。",
                  "沒有可顯示的共享項目。",
                  "No shared items to display.",
                ),
              )
        }}
      </p>
