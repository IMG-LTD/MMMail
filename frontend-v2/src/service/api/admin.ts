...
export async function readAdminSummary(options: AdminRequestOptions) {
  const response = await httpClient.get<ApiResponse<AdminSummary>>('/api/v2/admin/summary', options)
...
export function patchAdminUser(userId: string, body: Record<string, unknown>, options: AdminRequestOptions) {
  return httpClient.patch<ApiResponse<AdminUser>>(`/api/v2/admin/users/${userId}`, { ...options, body })
}
...
export async function listAdminDomains(options: AdminRequestOptions) {
  const response = await httpClient.get<ApiResponse<AdminDomain[]>>('/api/v2/admin/domains', options)
  return unwrapResponse(response)
}

export async function listAdminPolicies(options: AdminRequestOptions) {
  const response = await httpClient.get<ApiResponse<AdminPolicy[]>>('/api/v2/admin/policies', options)
  return unwrapResponse(response)
}

export function patchAdminPolicy(policyId: string, body: Record<string, unknown>, options: AdminRequestOptions) {
  return httpClient.patch<ApiResponse<AdminPolicy>>(`/api/v2/admin/policies/${policyId}`, { ...options, body })
}

export async function listAdminAudit(options: AdminRequestOptions) {
  const response = await httpClient.get<ApiResponse<AdminAuditEntry[]>>('/api/v2/admin/audit', options)
  return unwrapResponse(response)
}
...
export async function readAdminSystem(options: AdminRequestOptions) {
  const response = await httpClient.get<ApiResponse<AdminSystemStatus>>('/api/v2/admin/system', options)
  return unwrapResponse(response)
}

export async function readAdminRisk(options: AdminRequestOptions) {
  const response = await httpClient.get<ApiResponse<AdminRiskOverview>>('/api/v2/admin/risk', options)
  return unwrapResponse(response)
}
