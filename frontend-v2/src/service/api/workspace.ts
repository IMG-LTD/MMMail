...
export async function readWorkspaceSummary(token?: string) {
  const response = await httpClient.get<ApiResponse<WorkspaceSummary>>(
...
export async function listWorkspaceActivity(token?: string) {
  const response = await httpClient.get<ApiResponse<WorkspaceActivityItem[]>>(
...
export async function listWorkspaceTasks(token?: string) {
  const response = await httpClient.get<ApiResponse<WorkspaceTask[]>>("/api/v2/workspace/tasks", {
    token,
  });
  return unwrapResponse(response);
}
...
export async function patchWorkspaceTask(
  taskId: string,
  payload: PatchWorkspaceTaskPayload,
  token?: string,
) {
  const response = await httpClient.patch<ApiResponse<WorkspaceTask>>(
    `/api/v2/workspace/tasks/${taskId}`,
    {
      body: payload,
      token,
    },
