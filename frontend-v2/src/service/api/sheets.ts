...
export function readSheetsWorkbook(workbookId: string, token: string) {
  return httpClient.get<ApiResponse<SheetsWorkbookDetail>>(`/api/v2/sheets/${workbookId}`, {
    token,
  });
}

export function updateSheetsWorkbookCells(
  workbookId: string,
  body: Record<string, unknown>,
  token: string,
) {
  return httpClient.patch<ApiResponse<SheetsWorkbookDetail>>(`/api/v2/sheets/${workbookId}`, {
    body,
    token,
  });
}
...
export function importSheetsWorkbook(workbookId: string, body: SheetsImportPayload, token: string) {
  return httpClient.post<ApiResponse<SheetsWorkbookDetail>>(
    `/api/v2/sheets/${workbookId}/imports`,
    { body, token },
  );
}

export function createSheetsCleaningRule(
  workbookId: string,
  body: SheetsCleaningRulePayload,
  token: string,
) {
  return httpClient.post<ApiResponse<SheetsWorkbookDetail>>(
    `/api/v2/sheets/${workbookId}/cleaning-rules`,
    { body, token },
  );
}

export function listSheetsInsights(workbookId: string, token: string) {
  return httpClient.get<ApiResponse<SheetsInsight[]>>(`/api/v2/sheets/${workbookId}/insights`, {
    token,
  });
}
