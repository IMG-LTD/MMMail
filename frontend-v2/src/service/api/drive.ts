...
export function listDriveShares(itemId: string, token: string) {
  const fileId = itemId;
  return httpClient.get<ApiResponse<DriveShareLink[]>>(`/api/v2/drive/files/${fileId}/share`, {
    token,
  });
}
...
export function updateDriveFile(fileId: string, payload: DriveFileUpdatePayload, token: string) {
  return httpClient.patch<ApiResponse<DriveItem>>(`/api/v2/drive/files/${fileId}`, {
    body: payload,
    token,
  });
}
...
export function listDriveFileVersions(fileId: string, token: string) {
  return httpClient.get<ApiResponse<DriveFileVersion[]>>(`/api/v2/drive/files/${fileId}/versions`, {
    token,
  });
}
