...
export async function readBillingSummary(options: BillingRequestOptions) {
  const response = await httpClient.get<ApiResponse<BillingSummary>>('/api/v2/billing/summary', options)
  return unwrapResponse(response)
}

export async function listBillingPlans(options: BillingRequestOptions) {
  const response = await httpClient.get<ApiResponse<BillingPlan[]>>('/api/v2/billing/plans', options)
  return unwrapResponse(response)
}

export async function listBillingInvoices(options: BillingRequestOptions) {
  const response = await httpClient.get<ApiResponse<BillingInvoice[]>>('/api/v2/billing/invoices', options)
  return unwrapResponse(response)
}

export async function readBillingUsage(options: BillingRequestOptions) {
  const response = await httpClient.get<ApiResponse<BillingUsage>>('/api/v2/billing/usage', options)
  return unwrapResponse(response)
}
