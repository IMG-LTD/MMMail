...
export async function login(payload: LoginRequest) {
  const response = await httpClient.post<ApiResponse<AuthPayload>>("/api/v2/auth/login", {
    body: payload,
  });
...
export async function register(payload: RegisterRequest) {
  const response = await httpClient.post<ApiResponse<AuthPayload>>("/api/v2/auth/register", {
    body: payload,
  });
  return unwrapResponse(response);
}
...
export async function listSessions(token?: string) {
  const response = await httpClient.get<ApiResponse<UserSession[]>>("/api/v2/auth/sessions", {
    token,
  });
  return unwrapResponse(response);
}
