...
export const useOrgAccessStore = defineStore("org-access", () => {
  ...
  const activeScopeId = ref(
    canUseLocalStorage() ? window.localStorage.getItem(SCOPE_STORAGE_KEY) || "" : "",
  );
  const initialized = ref(false);
  const loading = ref(false);

  const activeScope = computed(
    () => accessScopes.value.find((scope) => scope.orgId === activeOrgId.value) || null,
  );
  ...
  const enabledProductSet = computed(() => {
    ...
    return new Set(
      items.filter((item) => item.accessState === "ENABLED").map((item) => item.productKey),
    );
  });
