...
test("round6 mail and shell surfaces protect mobile readability and edge targets", async () => {
  const [mailCss, topBar] = await Promise.all([
  ...
  assert.match(
    mailCss,
    /\.mail-message-list__row-head strong\s*\{[\s\S]*text-overflow:\s*ellipsis/,
  );
  assert.match(mailCss, /\.mail-message-list__row-head span\s*\{[\s\S]*flex:\s*0 0 auto/);
  assert.match(
    mailCss,
    /@media \(max-width: 1100px\)[\s\S]*border-bottom:\s*1px solid var\(--mm-border-strong\)/,
  );
  assert.match(
    topBar,
    /@media \(max-width: 820px\)[\s\S]*\.top-bar__right\s*\{[\s\S]*padding-right:\s*4px/,
  );
  assert.match(
    topBar,
    /@media \(max-width: 820px\)[\s\S]*\.avatar-chip\s*\{[\s\S]*min-width:\s*34px/,
  );
});
...
test("round6 workspace cards and tables expose touch-safe affordances", async () => {
  ...
  assert.match(
    calendarCss,
    /@media \(max-width: 820px\)[\s\S]*\.calendar-board__event\s*\{[\s\S]*padding:\s*10px/,
  );
  ...
  assert.match(
    dataTable,
    /@media \(max-width: 900px\)[\s\S]*\.data-table td button\s*\{[\s\S]*min-height:\s*40px/,
  );
...
test("round6 secondary workspace spacing leaves overflow and bottom-nav breathing room", async () => {
  const [adminView, collaborationView, commandCenterView, docsWorkspace, sheetsCss] =
    await Promise.all([
  ...
  assert.match(
    docsWorkspace,
    /@media \(max-width: 820px\)[\s\S]*\.docs-shell__list\s*\{[\s\S]*padding-bottom:\s*96px/,
  );
  assert.match(
    sheetsCss,
    /@media \(max-width: 980px\)[\s\S]*\.sheets-editor__actions::after\s*\{[\s\S]*linear-gradient/,
  );
  assert.match(
    collaborationView,
    /@media \(max-width: 820px\)[\s\S]*\.collaboration-activity\s*\{[\s\S]*margin-bottom:\s*96px/,
  );
  assert.match(commandCenterView, /class="page-shell surface-grid command-center-page"/);
  assert.match(
    commandCenterView,
    /@media \(max-width: 820px\)[\s\S]*\.command-center-page\s*\{[\s\S]*gap:\s*20px/,
  );
  assert.match(
    adminView,
