...
test("pass workspace reads items, mailboxes, and secure links from APIs", async () => {
  ...
  assert.match(api, /\/api\/v2\/pass\/items/);
  assert.match(api, /\/api\/v2\/pass\/vaults/);
  assert.match(api, /\/api\/v2\/pass\/share/);
  assert.match(api, /\/api\/v2\/pass\/secure-links/);
  assert.match(api, /\/api\/v2\/pass\/aliases/);
  assert.match(api, /\/api\/v2\/pass\/monitor/);
  assert.match(api, /listPassVaults/);
  assert.match(api, /createPassItem/);
  assert.match(api, /patchPassItem/);
  assert.match(api, /sharePassItem/);
  assert.match(api, /listPassSecureLinks/);
  assert.match(api, /deletePassSecureLink/);
  assert.match(api, /listPassAliases/);
  assert.match(api, /patchPassAlias/);
  assert.doesNotMatch(api, /\/api\/v1\/pass/);
...
test("pass runtime derivation merges monitor issues and exposes capped item coverage", async () => {
  const { PASS_ITEMS_FETCH_LIMIT, createPassMonitorIssueMap, derivePassSectionState } =
    await loadTsModule(stateFile);
