...
const aggregationFiles = [new URL("../src/views/app/NotificationsView.vue", import.meta.url)];
...
test("workspace notifications and collaboration surfaces consume shared v2 clients", async () => {
  const docsAndSheetsContents = await Promise.all(
    docsAndSheetsFiles.map((file) => readFile(file, "utf8")),
  );
  const aggregationContents = await Promise.all(
    aggregationFiles.map((file) => readFile(file, "utf8")),
  );
