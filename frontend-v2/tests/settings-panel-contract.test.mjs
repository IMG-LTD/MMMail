...
test('settings workspace keeps the frozen route-driven panel contract', async () => {
  ...
  assert.match(content, /getting-started[\s\S]*privacy-telemetry[\s\S]*system-health[\s\S]*integrations/)
  ...
  assert.match(content, /activePanelKey === 'getting-started'[\s\S]*新手引导|activePanelKey === 'getting-started'[\s\S]*Getting Started/i)
