...
test("settings workspace keeps the frozen route-driven panel contract", async () => {
  ...
  assert.match(
    content,
    /getting-started[\s\S]*privacy-telemetry[\s\S]*system-health[\s\S]*integrations/,
  );
  ...
  assert.match(content, /onboardingStore\.openGuide\(\)/);
  assert.match(content, /activePanelKey === 'getting-started'/);
  ...
  assert.match(
    content,
    /activePanelKey === 'getting-started'[\s\S]*新手引导|activePanelKey === 'getting-started'[\s\S]*Getting Started/i,
  );
  assert.match(
    content,
    /<section v-if="activePanelKey === 'privacy-telemetry'" class="settings-panel">[\s\S]*已注册设备/,
  );
  assert.match(
    content,
