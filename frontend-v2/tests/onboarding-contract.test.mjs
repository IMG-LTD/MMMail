...
function extractBalancedSource(content, startIndex, openCharacter, closeCharacter) {
  ...
  assert.fail(
    `Expected balanced ${openCharacter}${closeCharacter} source starting at index ${startIndex}`,
  );
}
...
function extractTargetPathValues(stepSource) {
  return Array.from(
    stepSource.matchAll(/\btargetPath\s*:\s*(["'`])([^"'`]+)\1/g),
    (match) => match[2],
  );
}
...
function hasAutoOpenGateContract(block) {
  ...
  const baseLayoutPattern =
    /(?:(?:route\.meta\??\.layout|layoutName|currentLayout)(?:\.value)?\s*(?:={2,3}|!==?)\s*['"`]base['"`]|['"`]base['"`]\s*(?:={2,3}|!==?)\s*(?:route\.meta\??\.layout|layoutName|currentLayout)(?:\.value)?|\bisBaseLayout(?:\.value)?\b)/;
  const openGuidePattern = /\bopenGuide\s*\(\s*\)/;
  const gateSignalPattern = /\bif\s*\(|&&|\|\|/;

  return (
    onboardingStatePattern.test(block) &&
...
function hasOnboardingAutoOpenGate(content) {
  return [...extractIfBlocks(content), ...extractReactiveCallBlocks(content)].some(
    hasAutoOpenGateContract,
  );
}
