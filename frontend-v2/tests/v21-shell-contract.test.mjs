...
test("v2.1 navigation model includes structured canonical and fallback product entries", async () => {
  const { findShellSurface, getToneColorVar, isMailRoute, mobilePrimaryTabs, shellNavGroups } =
    await loadTsModule(files.shellNav, {
      "@/locales": createLocaleRuntime(),
    });
  ...
  assert.deepEqual(Array.from(itemByKey.get("admin").matchPrefixes), [
    "/admin",
    "/organizations",
    "/business",
    "/security",
