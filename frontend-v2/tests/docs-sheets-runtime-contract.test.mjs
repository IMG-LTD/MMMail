...
const routeBoundEditorState = new URL(
  "../src/views/app/route-bound-editor-state.ts",
  import.meta.url,
);
...
test("docs and sheets surfaces read runtime workspaces instead of placeholder arrays", async () => {
  const [
    ...
    docsEditorContent,
    sheetsWorkspaceContent,
    sheetsEditorContent,
    routeBoundEditorStateContent,
  ] = await Promise.all([
  ...
  assert.match(
    docsApiContent,
    /updateDocsNote\(noteId: string, body: Record<string, unknown>, token: string\)/,
  );
  assert.match(docsApiContent, /\/api\/v2\/docs/);
  assert.match(docsApiContent, /\/api\/v2\/docs\/\$\{noteId\}/);
  assert.doesNotMatch(docsApiContent, /\/api\/v1\/docs/);
  ...
  assert.match(
    sheetsApiContent,
    /updateSheetsWorkbookCells\(workbookId: string, body: Record<string, unknown>, token: string\)/,
  );
  assert.match(sheetsApiContent, /\/api\/v2\/sheets/);
  assert.match(sheetsApiContent, /\/api\/v2\/sheets\/\$\{workbookId\}/);
  assert.doesNotMatch(sheetsApiContent, /\/api\/v1\/sheets/);
