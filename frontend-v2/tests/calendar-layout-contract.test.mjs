...
test("calendar layout splits cross-midnight events across visible days", async () => {
  ...
  const positioned = resolvePositionedCalendarEvents(
    ...
    {
      days: [{ key: "2026-05-14" }, { key: "2026-05-15" }],
      firstSlotHour: 0,
      selectedEventId: "",
    },
  );
  ...
  assert.deepEqual(
    positioned.map((item) => item.style.gridColumn),
    ["2", "3"],
  );
  assert.deepEqual(
    positioned.map((item) => item.style.gridRow),
    ["25 / span 1", "2 / span 2"],
  );
});
...
test("calendar date helpers keep local wall-clock dates across DST windows", async () => {
  ...
  try {
    const { addDays, formatDateKey, formatLocalDateTime, startOfWeek } =
      await loadTsModule(helperFile);
