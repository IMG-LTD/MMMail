...
test("auth guard redirects protected routes and leaves public shares open", async () => {
  ...
  assert.equal(
    resolveAuthRedirect(
      { fullPath: "/mail/inbox?label=team", meta: { auth: "required" }, path: "/mail/inbox" },
      false,
    ),
    "/login?redirect=%2Fmail%2Finbox%3Flabel%3Dteam",
  );
  assert.equal(
    resolveAuthRedirect({ fullPath: "/inbox", meta: { layout: "base" }, path: "/inbox" }, false),
    "/login?redirect=%2Finbox",
  );
  assert.equal(
    resolveAuthRedirect(
      { fullPath: "/share/mail/t1", meta: { layout: "blank" }, path: "/share/mail/t1" },
      false,
    ),
    null,
  );
  assert.equal(
    resolveAuthRedirect(
      { fullPath: "/mail/inbox", meta: { auth: "required" }, path: "/mail/inbox" },
      true,
    ),
    null,
  );
